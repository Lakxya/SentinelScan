...
SentinelScan provides eight scan entrypoints:

1. **`sentinelscan scan <path>`**: Runs target discovery and executes all active registered scanners (`SecretScanner`, `SastScanner`, `IacScanner`, `ScaScanner`, `DockerScanner`, `KubernetesScanner`, `AwsScanner`).
...
8. **`sentinelscan aws <path>`**: Runs target discovery and executes dedicated `AwsScanner` static AWS policy and posture analysis.
...
- `test_aws_scanner.py`: Validates IAM policy document structural validation (`Version` and `Statement`), Statement declared as object or list, wildcard Action, sensitive wildcard Resource, `iam:PassRole` wildcard, S3 public policy with/without restrictive conditions, S3 SSL transport enforcement, KMS wildcard principal, explicit `Effect: Deny` exclusions, local `.aws/credentials` key masking via `mask_token()`, elevated AWS config MFA profile detection, non-AWS JSON/YAML file exclusions, zero AWS API/CLI calls, and JSON report formatting.

---

## 🎯 6. Next Steps

- **Milestone 9 (Risk Correlation & Attack-Path Analysis)**: Finding correlation engine, posture scoring, and remediation reports.
